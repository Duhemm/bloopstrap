...
object TypesafeConfig2Class {
  def gimmeConfFromString(string: String, fallback: Config): Configured[Conf] =
    gimmeSafeConf(
      () => ConfigFactory.parseString(string).withFallback(fallback))
  def gimmeConfFromFile(file: java.io.File,
                        fallback: Config): Configured[Conf] = {
    if (!file.exists())
      ...
      gimmeSafeConf(() => ConfigFactory.parseFile(file).withFallback(fallback))
  }
  ...
  private def getPositionOpt(
    ...
    for {
      ...
      offsetByLine = cache.getOrElseUpdate(input,
